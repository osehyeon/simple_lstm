...
sequence_length = 10  # 원하는 sequence_length 값을 이곳에 입력하세요.
hidden_dim = 1       # 원하는 hidden_dim 값을 이곳에 입력하세요.
...
# 2. 데이터셋 준비하기
def generate_data(seq_length=sequence_length, num_samples=1000):
    ...
    target_data = np.array(target_data, dtype=np.float32).reshape(-1, sequence_length)
...
# 3. 모델 학습하기
model = LSTMModel(input_dim=1, hidden_dim=hidden_dim)
...
filename = f"lstm-{sequence_length}-{hidden_dim}.onnx"  # 자동으로 파일 이름을 생성
dummy_input = torch.randn(1, sequence_length, 1)  # 입력의 형태를 정의합니다.
