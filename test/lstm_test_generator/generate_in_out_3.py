...
def generate_model(rnn_type, input_dim, hidden_dim, bidirectional, layers, model_name, batch_one=True,
    ...
    for i in range(layers):
        ...
        X = 'layer_output_' + str(i)
    model.graph.output.add().CopyFrom(
        helper.make_tensor_value_info(X, onnx.TensorProto.FLOAT, [s_len, num_directions, 1 if batch_one else 'b', hidden_dim]))
    model = shape_inference.infer_shapes(model)
    onnx.save(model, model_name)

# rnn_type, input_dim, hidden_dim, bidirectional,  layers,   model_name, batch_one=True,  has_seq_len=False, s_len=1
generate_model("lstm", 1, 32, False, 1, "model.onnx", True, False, 10)

input = np.random.rand(1, 10, 1).astype(np.float32) 
